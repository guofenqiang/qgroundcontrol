...
#if 0
        // Old vales
        "enumStrings":      "System,български (Bulgarian),中文 (Chinese),Nederlands (Dutch),English,Suomi (Finnish),Fran?ais (French),Deutsche (German),Ελληνικ? (Greek), ????? (Hebrew),Italiano (Italian),日本語 (Japanese),??? (Korean),Norsk (Norwegian),Polskie (Polish),Português (Portuguese),Pусский (Russian),Espa?ol (Spanish),Svenska (Swedish),Türk (Turkish),Azerbaijani (Azerbaijani)",
